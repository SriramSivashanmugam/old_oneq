...
class _cotopaxicomSpider(Spider):
    ...
    start_urls = [
        'https://www.cotopaxi.com/sitemap.xml',
    ]
    ...
    def parse(self, response):
        ...
        product_links = list(filter(bool,[response.urljoin(e.strip()) for e in response.xpath('//a[contains(@class,"grid-view-item__link")]/@href').extract()]))
        ...
        pages = list(filter(bool,[response.urljoin(e.strip()) for e in response.xpath('//ul[contains(@class,"pagination")]//a[contains(@href,"page")]/@href').extract()]))
        for page in pages:
            if not page in self.foundlinks:
                self.foundlinks.append(page)
                yield scrapy.Request(page, callback=self.parse, meta={'category':response.url}, priority=1)

    def parse_product(self, response):
        # First make sure this is a product page check title of product if there is no title return
        name = next(iter(filter(bool,[e.strip() for e in response.xpath('//div[@class="product-title"]/h1/text()').extract()])),"")
        if not name:
        ...
        item["category"] = next(iter(filter(bool,[e.strip() for e in re.findall('"productType"]\s*=\s*\"(.*?)\"', response.text, re.DOTALL)])),"")
        item["price"] = next(iter(filter(bool,[e.replace('$','').strip() for e in response.xpath('//div[@class="product-title"]/p/text()').extract()])),"")
        if item['price'] == '':
            item["price"] = next(iter(filter(bool,[e.replace('$','').strip() for e in response.xpath('//div[@class="product-title"]/p/s/text()').extract()])),"")
            item["sales_price"] = next(iter(filter(bool,[e.replace('$','').strip() for e in response.xpath('//div[@class="product-title"]/p/span/text()').extract()])),"")
        item["colors"] = list(set(list(filter(bool,[e.strip() for e in re.findall('<button\s*type=\"button\"\s*aria-label=\"product-color-button\"\s*data-color=\"([^"]*?)\"(?:\>|\s*class="selected">)',response.text,re.DOTALL)])))) # colour will be populated with another request
        if item['colors'] == []:item["colors"] =  list(set(list(filter(bool,[e.strip() for e in response.xpath('//div[@class="option-block color"]/p/span/text()').extract()]))))
        item["sizes"] = list(set(list(filter(bool,[e.strip() for e in re.findall('<button\s*type=\"button\"\s*aria-label=\"Product Size Button\"[^>]*?>\s*<span>([^>]*?)<\/span>',response.text,re.DOTALL)])))) # necklace_length will be populated with another request
        item["reviews"] = list(set(list(filter(bool,[e.strip() for e in response.xpath('//div[@class="yotpo-review-wrapper"]/div[@class="content-review"]/text()').extract()])))) # text as a list 
        reviews_count = next(iter(filter(bool,[e.strip().replace('Reviews','').replace('Review','').replace('Write a review','') for e in response.xpath('//div[@class="yotpo-bottomline pull-left  star-clickable"]/a/text()').extract()])),"")
        if reviews_count!='':item["reviews_count"] = int(reviews_count)# total review count as integer
        item["rating"] = next(iter(filter(bool,[e.replace('star rating','').strip() for e in response.xpath('//span[@class="yotpo-stars"]/span[@class="sr-only"]/text()').extract()])),"") # star or review rating of product as string
        item["images"] = list(set(list(filter(bool,[e.strip() for e in response.xpath('//nav[@class="product-thumbnails"]/figure/span/img/@src').extract()]))))
        item["images"] = [urljoin(url,img_url) for img_url in item["images"]]
        # Finally yielding item
        yield item

#Output {'url':'https://www.cotopaxi.com/products/on-the-horizon-t-shirt-mens?variant=39382956408893','name': "On The Horizon T-Shirt - Men's", 'category': 'Apparel', 'price': '$30', 'colors': ['Rust'], 'sizes': [ 'S'], 'reviews': ['I wore it for the first time hiking in RMNP and it was great'], 'reviews_count': 1, 'rating': '5.0 ', 'images': ['http://cdn.shopify.com/s/files/1/0281/7544/products/1200x1200png-f21_m_on_the_horizon_tee_heather_gray_back_1024x1024.png?v=1631912462', 'http://cdn.shopify.com/s/files/1/0281/7544/products/1200x1200png-f21_m_on_the_horizon_tee_maritime_front_1024x1024.png?v=1631912401', 'http://cdn.shopify.com/s/files/1/0281/7544/products/1200x1200png-f21_m_on_the_horizon_tee_heather_gray_front_1024x1024.png?v=1631912462']}