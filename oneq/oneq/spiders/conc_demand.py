...
class _allegroSpider(Spider):
    ...
    def parse(self, response):
        ...
        product_links = list(
            filter(
                bool,
                [response.urljoin(e.strip()) for e in response.xpath("").extract()],
            )
        )
                ...
                filter(
                    ...
                    [
                        ...
                        for e in response.xpath(
                            '//h1[contains(@class,"product-name mt1")]//text()'
                        ).extract()
                    ],
                )
            ),
            ...
            iter(
                filter(
                    ...
                    [
                        e.strip()
                        for e in response.xpath(
                            '//div[@class="desktop-content"]//p[@class="affirm-as-low-as"]/@data-category'
                        ).extract()
                    ],
                )
            ),
            ...
            iter(
                filter(
                    ...
                    [
                        e.strip()
                        for e in response.xpath(
                            '//div[@class="desktop-content"]//span[@itemprop="price"]//text()'
                        ).extract()
                    ],
                )
            ),
                ...
                list(
                    filter(
                        ...
                        [
                            e.strip()
                            for e in response.xpath(
                                '//ul[@class="options swatches color"]/li/@data-selection-name'
                            ).extract()
                        ],
                    )
                )
                ...
                list(
                    filter(
                        ...
                        [
                            e.strip()
                            for e in response.xpath(
                                '//li[contains(@class,"size-swatch")]/span//text()'
                            ).extract()
                        ],
                    )
                )
        ...
        item["reviews_count"] = next(
            iter(
                filter(
                    bool,
                    [
                        e.lower().replace("reviews", "").replace("review", "").strip()
                        for e in response.xpath(
                            '//span[contains(@class,"reviews-qa-label ")]//text()'
                        ).extract()
                    ],
                )
            ),
            ...
            iter(
                filter(
                    ...
                    [
                        e.strip()
                        for e in response.xpath(
                            '//span[contains(@class,"avg-score")]//text()'
                        ).extract()
                    ],
                )
            ),
            ...
            set(
                list(
                    filter(
                        bool,
                        [
                            e.replace("sw=250&sh=250", "sw=2000&sh=2000").strip()
                            for e in response.xpath(
