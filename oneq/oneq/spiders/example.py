...
class _cotopaxicomSpider(Spider):
    ...
    def parse_product(self, response):
        ...
        item["category"] = next(iter(filter(bool,[e.strip() for e in re.findall('"productType"]\s*=\s*\"(.*?)\"', response1, re.DOTALL)])),"")
        ...
        item["colors"] = list(set(list(filter(bool,[e.strip() for e in re.findall('<button\s*type=\"button\"\s*aria-label=\"product-color-button\"\s*data-color=\"([^"]*?)\"(?:\>|\s*class="selected">)',str(response1),re.DOTALL)])))) # colour will be populated with another request
        ...
        item["sizes"] = list(set(list(filter(bool,[e.strip() for e in re.findall('<button\s*type=\"button\"\s*aria-label=\"Product Size Button\"[^>]*?>\s*<span>([^>]*?)<\/span>',response1,re.DOTALL)])))) # necklace_length will be populated with another request
